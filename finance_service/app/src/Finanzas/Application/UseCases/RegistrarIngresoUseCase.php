...
/**
 * RegistrarIngresoUseCase - Caso de Uso
 * 
 * Orquesta el flujo para registrar un ingreso (venta) en una caja.
 * 
 * Responsabilidades:
 * 1. Validar entrada
 * 2. Buscar la caja existente
 * 3. Registrar el ingreso en el agregado
 * 4. Persistir los cambios
 * 
 * @package Finanzas\Application\UseCases
 */
final class RegistrarIngresoUseCase
{
    /**
    ...
    /**
     * Ejecutar el caso de uso: registrar un ingreso
     *
     * @param string $cajaId ID de la caja donde registrar el ingreso
     * @param float $montoPen Monto del ingreso en PEN
     * @param string $descripcion Descripción del ingreso (ej: "Venta de 3 donas")
     * @return void
     * 
    ...
    public function execute(string $cajaId, float $montoPen, string $descripcion): void
    {
        ...
        // 4. Registrar el ingreso en el agregado
        // (El agregado valida automáticamente que esté abierta)
        $caja->registrarIngreso($monto, $descripcion);
    ...
    private function validarMonto(float $monto): void
    {
        ...
        if ($monto > 100000) {
            throw new InvalidArgumentException('El monto del ingreso no puede exceder 100000 PEN');
        }
    }
