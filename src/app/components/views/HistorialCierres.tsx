...
import { Search, Eye, Calendar, User, Coins, TrendingUp, TrendingDown, Download, Package } from "lucide-react";
...
interface Cierre {
  ...
  movimientos: any[];
  insumosSnapshot?: { id: number; nombre: string; categoria: string; stockAlCierre: number }[];
}
...
export function HistorialCierres() {
  ...
  const exportInsumosCSV = (cierre: Cierre) => {
    const snapshot = cierre.insumosSnapshot;
    if (!snapshot || snapshot.length === 0) {
      alert("Este cierre no tiene snapshot de insumos guardado.");
      return;
    }
    const header = "Insumo,Categoría,Stock al Cierre";
    const rows = snapshot.map(row =>
      `"${row.nombre}","${row.categoria}",${row.stockAlCierre}`
    );
    ...
    a.href = url;
    a.download = `insumos_cierre_${cierre.fecha}_${cierre.hora.replace(/:/g, "-")}.csv`;
    a.click();
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          {viewingCierre && (
            <div className="space-y-6">
                ...
                <CardContent>
                  <div className="text-center">
                    ...
                    <p className="text-sm text-muted-foreground mt-2">
                      Calculado: Total Contado - Fondo Inicial - Egresos
                    </p>
                  </div>
                </CardContent>
              </Card>
              ...
              {/* Inventario de Insumos al Cierre */}
              <Card>
                <CardHeader>
                  <div className="flex items-center justify-between">
                    ...
                    {viewingCierre.insumosSnapshot && viewingCierre.insumosSnapshot.length > 0 && (
                      <Button size="sm" variant="outline" onClick={() => exportInsumosCSV(viewingCierre)}>
                ...
                <CardContent>
                  {!viewingCierre.insumosSnapshot || viewingCierre.insumosSnapshot.length === 0 ? (
                    <p className="text-sm text-muted-foreground text-center py-4">
                      No hay snapshot de insumos para este cierre
                    </p>
                  ) : (
                      ...
                      <Table>
                        <TableHeader>
                          <TableRow>
                            ...
                            <TableHead className="text-right">Stock al Cierre</TableHead>
                          </TableRow>
                        </TableHeader>
                        <TableBody>
                          {viewingCierre.insumosSnapshot.map(ins => (
                            <TableRow key={ins.id}>
                              <TableCell className="font-medium">{ins.nombre}</TableCell>
                              <TableCell className="text-muted-foreground">{ins.categoria}</TableCell>
                              <TableCell className={`text-right font-medium ${
                                ins.stockAlCierre === 0 ? "text-red-600" :
                                ins.stockAlCierre < 10 ? "text-orange-500" : ""
                              }`}>
                                {ins.stockAlCierre} und.
                              </TableCell>
                            </TableRow>
                          ))}
                        </TableBody>
