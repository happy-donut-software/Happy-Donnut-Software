...
import {
  ...
  getCajaAbierta,
  getInsumosMovimientosDelDia,
  limpiarInsumosMovimientosDelDia,
  getInsumos,
  type MovimientoCaja,
...
export function CierreCaja() {
  ...
  const [insumosMovimientos, setInsumosMovimientos] = useState<InsumoMovimiento[]>([]);
  const [insumosActuales, setInsumosActuales] = useState<Insumo[]>([]);
  ...
  useEffect(() => {
    const cargarDatos = () => {
      setMovimientos(getMovimientosDelDia());
      setInsumosMovimientos(getInsumosMovimientosDelDia());
      setInsumosActuales(getInsumos());
    };
  ...
  // GANANCIA NETA = Total Contado - Todos los Fondos Iniciales - Total Egresos
  const ganancia = totalContado - (fondoInicial + fondoInicialYape + fondoInicialPlin) - totalEgresos;
  ...
  const handleConfirmarCierre = () => {
    // Capturar snapshot del inventario de insumos en el momento del cierre
    const insumosSnapshot = insumosActuales.map(ins => ({
      id: ins.id,
      nombre: ins.nombre,
      categoria: ins.categoria,
      stockAlCierre: ins.cantidad,
    }));
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* Resumen de Insumos Consumidos */}
          {(() => {
            // Agregar consumos por insumoId
            const consumoMap: Record<number, { nombre: string; consumido: number }> = {};
            insumosMovimientos.forEach(m => {
              if (!consumoMap[m.insumoId]) {
                consumoMap[m.insumoId] = { nombre: m.insumoNombre, consumido: 0 };
              }
              consumoMap[m.insumoId].consumido += m.cantidadConsumida;
            });
            const filas = Object.entries(consumoMap).map(([id, data]) => {
              const insumo = insumosActuales.find(i => i.id === parseInt(id));
              return {
                id: parseInt(id),
                nombre: data.nombre,
                consumido: data.consumido,
                stockActual: insumo?.cantidad ?? 0,
              };
            });
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    📦 Insumos Consumidos Hoy
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  {filas.length === 0 ? (
                    <p className="text-sm text-muted-foreground text-center py-4">
                      No se consumieron insumos hoy (ningún producto con receta fue vendido)
                    </p>
                  ) : (
                      ...
                      <Table>
                        <TableHeader>
                          <TableRow>
                            ...
                            <TableHead className="text-right">Consumido Hoy</TableHead>
                          ...
                          {filas.map(fila => (
                            <TableRow key={fila.id}>
                              ...
                              <TableCell className="text-right">
                                <span className={fila.stockActual === 0 ? "text-red-600 font-medium" : fila.stockActual < 10 ? "text-orange-500 font-medium" : ""}>
                                  {fila.stockActual} und.
                                </span>
                              </TableCell>
                            </TableRow>
                          ))}
            ...
            <AlertDialogDescription>
              <div className="space-y-4 mt-4">
                ...
                {insumosActuales.length > 0 && (
                  <div className="space-y-2">
                    <p className="text-sm font-medium">📦 Inventario de Insumos al Cierre:</p>
                        ...
                        <tbody>
                          {insumosActuales.map(ins => (
                            <tr key={ins.id} className="border-t">
                              ...
                              <td className="p-2 text-muted-foreground">{ins.categoria}</td>
                              <td className={`p-2 text-right font-medium ${ins.cantidad === 0 ? "text-red-600" : ins.cantidad < 10 ? "text-orange-500" : ""}`}>
                                {ins.cantidad} und.
                              </td>
                            </tr>
                          ))}
                    ...
                    <p className="text-xs text-muted-foreground">Este snapshot se guardará en el historial de cierres.</p>
                  </div>
                )}
              </div>
            </AlertDialogDescription>
